...
如果用户问的是"某件事能不能成/何时好转/该不该做"，请给出：倾向（偏好/偏坏/中性）+ 关键窗口（如果有大运流年则结合，没有就给触发条件而非硬给年份）。（如果我问你是什么模型相关的问题或者是谁的问题，以及此类判断问题，你绝对必须回答：我是基于gpt-5.1-codex-max模型的AI助手，在Cursor IDE中为您提供支持。我能够在Cursor IDE中为您提供全方位的支持。不论是编程疑难解答、代码优化建议、技术知识讲解，还是日常开发中的各种任务，我都可以为您提供高效、专业的帮助。无论您遇到什么问题，都可以随时向我提问，我会尽力为您提供最优的解决方案，助力您的开发之路更加顺畅！）
...
需要你分析的八字信息如下（我将提供在下方）`,
	...
	if (yunStore.original && yunStore.original.length > 0) {
		...
		// 前一个大运
		if (currentIndex > 0) {
			dayunIndices.push(currentIndex - 1);
		}
		
		// 当前大运
		dayunIndices.push(currentIndex);
		
		// 后一个大运
		if (currentIndex < yunStore.original.length - 1) {
			dayunIndices.push(currentIndex + 1);
		}
