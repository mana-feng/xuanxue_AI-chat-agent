...
const limitedDayList = computed(() => {
	...
	// yun_store.day_list 已经在 store 中根据节气日期过滤好了，这里直接返回即可
	// 之前的过滤逻辑存在硬编码日期的问题，可能导致部分流日显示不出来
	console.log('limitedDayList computed', dayList.length);
	return dayList;
});
...
function ScrollItemClick(e: number, index: number | string) {
	...
	// 如果点击的是当前已选中的项，则取消选择
	if (currentIndex === idx) {
		(yun_store as any)[key_list[e]] = -1;
		
		// 清空下级数据
		if (e === 0) { // 大运
			yun_store.year_list = [];
			yun_store.month_list = [];
			yun_store.day_list = [];
			yun_store.year_index = -1;
			yun_store.month_index = -1;
			yun_store.day_index = -1;
		} else if (e === 1) { // 流年
			yun_store.month_list = [];
			yun_store.day_list = [];
			yun_store.month_index = -1;
