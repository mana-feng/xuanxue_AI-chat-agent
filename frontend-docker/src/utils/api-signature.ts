...
function base64UrlToJson(input: string): any | null {
	try {
...
async function generateSignature(params: any, timestamp: number, nonce: string): Promise<string> {
	...
	const sessionKey = getSessionSignatureKey();
	if (sessionKey) {
		return await hmacSha256Hex(signString, sessionKey);
	}
