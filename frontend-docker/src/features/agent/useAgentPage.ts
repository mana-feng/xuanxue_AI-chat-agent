...
export function useAgentPage() {
	...
	const accessLabel = computed(() => '匿名可用');
	const usageLabel = computed(() => '不检查额度');
	const conversationSubtitle = computed(
		() => 'Agent 会根据你的自然语言直接排盘、聚焦紫微宫位或给出下一步动作，无需登录即可直接调用',
	);
	...
	async function sendPrompt(text: string) {
		...
		try {
			...
			if (!res?.success) {
				throw new Error(res?.error || res?.message || '调用失败');
			}

			const rawReply = extractTextFromPayload(res?.data) || (res?.data as any)?.reply || '';
