...
export const useYunStore = defineStore('yun', {
	...
	actions: {
		...
		async resolveLiuDay() {
			...
			let startSolar = currentMonth.original as Solar;
			
			// 防御性编程：如果 startSolar 丢失了原型方法（可能是因为序列化），尝试重建
			if (!startSolar || typeof startSolar.getYear !== 'function') {
				try {
					if (currentMonth.year && currentMonth.date) {
						const [m, d] = currentMonth.date.split('/').map(Number);
						// 注意：Solar.fromYmd 月份是 1-12
						let y = currentMonth.year;
						// month_index 11 对应的是下一年（小寒），但 currentMonth.year 存储的是流年年份
						if (month_index === 11) {
							y++;
						}
						startSolar = Solar.fromYmd(y, m, d);
					}
				} catch (e) {
					// ignore error
				}
			...
			// 注意：Date 月份是 0-11
			const endSolar = Solar.fromDate(new Date(endYear, nextMonth - 1, nextDay)); 
		...
		async autoLocateToCurrentDate(skipAutoLocate = false): Promise<void> {
			...
			if (!skipAutoLocate && this.year_list && this.year_list.length > 0) {
				...
				// 处理年初（1月和2月立春前）属于上一年八字的逻辑
				if (currentMonth === 1) {
					targetYear = currentYear - 1;
				} else if (currentMonth === 2) {
					// 查找当年的立春日期
					let lichunDate: any = null;
					// 立春通常在2月3日-5日之间
					for (let d = 3; d <= 6; d++) {
						const s = Solar.fromYmd(currentYear, 2, d);
						if (s.getLunar().getJieQi() === '立春') {
							lichunDate = s;
							break;
						}
					}
					...
					if (lichunDate) {
						const currentSolar = Solar.fromYmd(currentYear, currentMonth, currentDay);
						if (currentSolar.isBefore(lichunDate)) {
							targetYear = currentYear - 1;
						}
					}
				}

				const yearIndex = this.year_list.findIndex((item: any) => item.year == targetYear);
			...
			if (this.month_list && this.month_list.length > 0) {
				...
				if (monthIndex >= 0 && (this.month_index !== monthIndex || !this.day_list.length)) {
