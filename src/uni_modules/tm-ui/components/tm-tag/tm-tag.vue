<template>
    <view :disabled="_attrs.disabled" @click="onClick" class="tmTag" :class="[_attrs.disabled ? 'tmTagLoading' : '']" :style="[
        ...
        <!--
          ...
          -->
        <slot>
            <text v-if="attrs.label" :style="{ color: buttonStyle.color || '#333333', fontWeight: attrs.bold ? 'bold' : 'normal' }">{{ attrs.label }}</text>
        </slot>
    </view>
</template>
...
const _fontSize = computed(() => {
    if (attrs.fontSize === '' || attrs.fontSize === null || attrs.fontSize === undefined) {
        return covetButtonSize('fontSize', attrs.size);
    }
    return covetUniNumber(attrs.fontSize);
})
