<template>
    <view :disabled="_attrs.disabled||_attrs.loading" class="tmButton" :class="[_attrs.loading || _attrs.disabled ? 'tmButtonLoading' : '']"
        ...
        -->
        <slot>
            <text v-if="!_attrs.btnIcon && _label" :style="{ color: buttonStyle.color || '#333333', fontWeight: _attrs.bold ? 'bold' : 'normal' }">{{ _label }}</text>
        </slot>
...
const attrs = defineProps({
    ...
    linear: {
        ...
        validator: (val: string[] | string) => {
            if (!val || (Array.isArray(val) && val.length == 0) || (typeof val === 'string' && val === '')) return true;
            if (typeof val === 'string') {
                // 字符串简写格式，如 'right', 'left' 等
...
const _label = computed(() => attrs.label || '')
...
const buttonStyle = computed(() => {
    ...
    // 支持字符串简写，如 'right', 'left' 等，转换为数组格式
    let linearValue: string[] = [];
    if (Array.isArray(attrs.linear) && attrs.linear.length >= 3) {
        linearValue = attrs.linear;
    } else if (typeof attrs.linear === 'string' && attrs.linear !== '') {
        // 将字符串简写转换为数组格式
        ...
        const direction = directionMap[attrs.linear] || 'to right';
        const colorValue = color || config.color || 'primary';
        const primaryColor = getDefaultColor(colorValue);
        const colorObj = getDefaultColorObj(colorValue);
        const secondaryColor = colorObj?.active?.background || primaryColor;
        linearValue = [direction, primaryColor, secondaryColor];
    }
    let linear = linearValue.length >= 3 ? linearValid(linearValue) : '';
