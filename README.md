...
**此外还有体验版→[xuan.manafeng.com](https://xuan.manafeng.com)，是我将后端拆除后，仅保留了前端然后挂载到 github pages 的一个版本。此版本的排盘信息与大语言模型 API Key 只保存在你自己浏览器的本地存储中，不会上传到任何服务器；你的提问只会直接发往你自己填写的模型服务商。提示：API Key 以明文形式存放在浏览器本地，请勿在公用电脑上保存，用完可在设置页一键清除。**

> 关于访问统计：体验版接入了第三方网页访问统计（用于了解访问量），会记录 IP、来源页面、浏览器与设备类型等常规访问信息。**它不接触你的排盘数据、对话内容和 API Key。** 如果介意，可用浏览器的拦截插件屏蔽，不影响任何功能。
