...
**此外还有体验版→[xuan.manafeng.com](https://xuan.manafeng.com)，是我将后端拆除后，仅保留了前端然后挂载到 github pages 的一个版本。此版本的所有数据（排盘信息、大语言模型 API Key 等）只保存在你自己浏览器的本地存储中，不会上传到任何服务器。提示：API Key 以明文形式存放在浏览器本地，请勿在公用电脑上保存，用完可在设置页一键清除。**
