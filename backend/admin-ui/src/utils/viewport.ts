...
const getMode = (width: number): LayoutMode => {
	if (width >= BREAKPOINT_WIDE) return 'layout-wide';
	if (width >= BREAKPOINT_MEDIUM) return 'layout-medium';
...
const getScale = (width: number) => {
	...
	if (width >= BREAKPOINT_MEDIUM) return SCALE_MAX;
	return clamp(width / BREAKPOINT_MEDIUM, SCALE_MIN, SCALE_MAX);
};
