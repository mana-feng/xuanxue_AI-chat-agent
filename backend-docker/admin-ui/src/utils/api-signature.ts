...
import { API_SIGNATURE_SECRET, API_SIGNATURE_ENABLED } from '@/config/config';
...
function getSessionSignatureKey(): string | null {
	const token = getAccessToken();
	if (!token) return null;
	const parts = String(token).split('.');
	if (parts.length < 2) return null;
	const payload = base64UrlToJson(parts[1]);
	const signatureKey = payload?.sk;
	return typeof signatureKey === 'string' && signatureKey.length > 0 ? signatureKey : null;
}
...
async function generateSignature(
	...
	const actualSecret = secret || API_SIGNATURE_SECRET;
	if (actualSecret && (secret || API_SIGNATURE_ENABLED)) {
		try {
			return await hmacSha256Hex(signString, actualSecret);
		} catch (e) {
...
export async function signRequest(
	...
	// 生成签名（简化版，实际验证在后端）
	const signature = await generateSignature(data, timestamp, nonce, getSessionSignatureKey() || undefined);
