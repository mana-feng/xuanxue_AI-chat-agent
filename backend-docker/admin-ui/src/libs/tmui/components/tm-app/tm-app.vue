<template>
	<view class="flex flex-col relative"
		:style="[appConfig.theme ? { background: appConfig.theme } : '', { width: '100vw', minWidth: '100vw', minHeight: appConfig.height + 'px' }]">
		<!-- #ifndef APP-NVUE -->
		<!-- 	<image
...
<script lang="ts" setup>
	...
	// #ifdef H5
	const handleWindowResize = (res?: UniApp.OnWindowResizeResult) => {
		const width = res?.size?.windowWidth ?? uni.getSystemInfoSync().windowWidth;
		const height = res?.size?.windowHeight ?? uni.getSystemInfoSync().windowHeight;
