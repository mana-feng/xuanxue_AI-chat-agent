...
export const GUA64 = (() => {
	...
	for (let i = 0; i < trigramCodes.length; i++) {
		for (let j = 0; j < trigramCodes.length; j++) {
			...
			// 只存储卦名，不带 "天为乾" 这种描述，或者保持原有格式
			// 原有逻辑是：upper === lower ? `${upperLabel.element}为${hexName || upperLabel.name}` : `${upperLabel.element}${lowerLabel.element}${hexName || ''}`.trim();
			// 但前端代码似乎只用了 hexName。
			// 让我们保持与前端一致的逻辑，或者保持后端原有的 display 逻辑但名字要是对的。
			// 后端这里原来的代码是：
			...
			// result[mark] = display;
			// 修正后的 TRIGRAM_LABEL 和 hexName 应该是正确的。
			// 这里我们直接用 hexName 吧，或者保留 display 格式。
			// 考虑到前端可能直接显示 result[mark]，还是保留 display 格式比较安全，但要确保 hexName 是对的。
			// 不过前端最新的 GUA64 实现（我在上一步改的）是直接 result[mark] = hexName;
			// 让我们看看前端的实现。
			// 前端实现： result[mark] = hexName;
			// 后端实现之前是 result[mark] = display;
			// 为了保持一致性，且避免 "天火同人" 显示成 "天火同人" (重复?)
			// 如果 hexName 本身就是 "同人"，那么 display 就是 "天火同人"。
			// 如果 hexName 是 "乾为天"，display 是 "天为乾为天" (有点怪)。
			// 让我们统一成前端的逻辑：result[mark] = hexName;
			// 这样前后端一致。
			result[mark] = hexName;
		}
	}
